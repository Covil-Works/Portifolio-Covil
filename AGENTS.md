...
- `Tecnologias` deve permanecer como subseção ao final do bloco de `Projetos`, e não como seção principal independente. `Contato` deve permanecer como seção principal própria após esse bloco, com LinkedIn, GitHub, e-mail e WhatsApp; o link `Vamos conversar` continua apontando para `#contato`.
